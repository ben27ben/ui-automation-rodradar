...
test.only("Add new organization", async ({ page }) => {
  await dashboardPage.navigateAndVerify(
    "Organizations",
    "organizations/bucket-manufacture"
  );
  ...
  await page
    .getByRole("textbox", { name: "Bucket Manufacture Name" })
    .fill(cleanName);
  await page.getByRole("textbox", { name: "Phone Number" }).fill(phoneNumber);
  await page.getByText('Update only upcoming version').click();
...
test("Filter organizations by status", async ({ page }) => {
  await dashboardPage.navigateAndVerify(
    "Organizations",
    "organizations/bucket-manufacture"
  );
